...
// 20*1 append 12*0, used to align pages
#define PAGE_ALIGN (~((1 << PAGE_SHIFT) - 1))
#define MAX_INFO 10

struct bootmm {
	unsigned int phymm;		// actual physical memory
	unsigned int max_pfn;	// the max page number
	unsigned char* s_map;	// map begin place
	unsigned char* e_map;	// map end place
	unsigned int last_alloc_end;
	unsigned int cnt_infos;		// number of infos stored in bootmm
